...
from boxmot.utils.Similarity_Score_Calculation import SSIM_Cost
from boxmot.utils.feature_extractor import VLAD_Cost
...
def hm_associate(
    ...
    if min(iou_matrix.shape):
        ...
        if a.sum(1).max() == 1 and a.sum(0).max() == 1:
            ...
            if use_ssim:
                ssim_cost = SSIM_Cost(detection_img=detection_img, track_img=track_img)
                final_cost += ssim_cost * ssim_ratio
            if use_vlad:
                vlad_cost = VLAD_Cost(detection_img=detection_img, track_img=track_img)
                final_cost += vlad_cost * vlad_ratio
